...
新电脑或新聊天接手时，先阅读 [`PROJECT_HANDOFF.md`](PROJECT_HANDOFF.md)；模型来源、校验值和精确依赖见 [`docs/09_模型权重与依赖说明.md`](docs/09_模型权重与依赖说明.md)。前沿论文、成熟 GitHub 工程与本项目的差距对照见 [`docs/10_前沿方案对照与补强路线.md`](docs/10_前沿方案对照与补强路线.md)；三类补充数据的输入输出见 [`docs/12_三类真实补充数据输入输出说明.md`](docs/12_三类真实补充数据输入输出说明.md)。
