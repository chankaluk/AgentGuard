...
AgentGuard属于 AI 衍生安全方向，系统把 AI Agent 的输入、工具调用、记忆和权限行为，与主机进程、网络、文件、注册表事件统一编码为行为序列；实际告警链路采用轻量 Transformer 与透明顺序规则融合，并为每条告警输出关键事件、原始日志证据和自然语言解释。

> 数据声明：`data/demo` 是固定随机种子生成的自建增强工程基准，只用于证明代码、训练、评测和演示链路可复现，不能代表生产环境效果。项目同时披露简单基线、模型单独结果、规则单独结果和混合系统结果。

新电脑或新聊天接手时，先阅读 [`PROJECT_HANDOFF.md`](PROJECT_HANDOFF.md)；最终部署、演示流程和四个可上传 `.log` 样例见 [`docs/13_最终项目文档与部署说明.md`](docs/13_最终项目文档与部署说明.md)。模型来源、校验值和精确依赖见 [`docs/09_模型权重与依赖说明.md`](docs/09_模型权重与依赖说明.md)。前沿论文、成熟 GitHub 工程与本项目的差距对照见 [`docs/10_前沿方案对照与补强路线.md`](docs/10_前沿方案对照与补强路线.md)；三类补充数据的输入输出见 [`docs/12_三类真实补充数据输入输出说明.md`](docs/12_三类真实补充数据输入输出说明.md)。
...
现场上传演示可直接使用 `data/upload_logs/` 下的四个 `.log` 文件：公开 BGL 异常日志、可控异常链、本机正常快照和混合测试日志。页面会自动识别格式，上传后可点击“更换日志”继续选择其它文件。
