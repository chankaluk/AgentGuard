...
- 完整流水线已验证：数据生成、基线、训练、评测、34 项测试、材料生成、项目验证和最终打包全部成功。
...
4. Word：结构回读通过；主报告为 9 节、至少 2 张表和 3 幅图片，使用说明含 4 幅界面证据图，动态页码域和自动更新设置存在，作者元数据为空。本机无 LibreOffice；Microsoft Word 隐藏导出曾卡死，已终止对应隐藏进程且未损坏报告，因此不声称 Word 逐页视觉检查通过。
...
- 模型：`artifacts/agentguard.pt`
  - SHA-256：`D131C219FB00041EA5F96DCC38967346A6EFBDFC4BE4F7BC83D5E76659A8A684`
...
第二条命令会依次完成数据、训练、评测、测试、材料生成、项目验证与打包。
